...
class EntryQueries(Queries):
    ...
    def get_entry(self, id:str, account_id: str) -> MealEntry:
        meal = self.collection.find_one({'_id': ObjectId(id), 'account_id': account_id})
        meal['id'] = str(meal['_id'])
        return meal


    def update(self, id: str, params: MealParams, account_id: str) -> MealEntry:
        entry = params.dict()
        meal = self.collection.find_one_and_update({'_id': ObjectId(id), 'account_id': account_id}, {'$set': entry})
        meal['id'] = str(meal['_id'])
        return MealEntry(**meal)


    def delete(self, id: str, account_id: str) -> bool:
        result = self.collection.delete_one({'_id': ObjectId(id), 'account_id': account_id})
